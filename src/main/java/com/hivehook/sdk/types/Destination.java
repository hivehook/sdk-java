...
/**
 * Outbound destination that Hivehook delivers events to.
 *
 * <p>Nullable fields are documented per-component.
 *
 * @param id                source UUID.
 * @param name              human-readable name.
 * @param url               target URL.
 * @param signingSecret     signing secret used to HMAC outbound requests.
 * @param status            current status (e.g. {@code ACTIVE}, {@code DISABLED}).
 * @param type              destination type (defaults to {@code "HTTP"}).
 * @param typeConfig        type-specific configuration map (nullable).
 * @param timeoutMs         request timeout in milliseconds.
 * @param rateLimitRps      per-destination rate limit.
 * @param retryPolicy       retry policy (nullable).
 * @param headers           static request headers (nullable).
 * @param authType          auth strategy (e.g. {@code "BEARER"}, {@code "BASIC"}, {@code "OAUTH2"}).
 * @param oauth2Config      OAuth2 config when {@code authType} is {@code OAUTH2} (nullable).
 ...
 * @param outputFormat      output format identifier (defaults to {@code "default"}).
